class Person:
    ...
    def __init__(self, name: str = 'Unknown', age: int = 1): # all private methods are with __
        self.__name = None # private variables with __
        self.__age = None
        self.set_name(name)
        self.set_age(age)
    ...
    def get_name(self) -> str: # getter
        if self.__name is None or self.__name == '' or type(self.__name) != str:
            raise RuntimeError(f'Invalid name has been set: {self.__name}') # throw to exception
